...
POP_SIZE, GENS = 50, 50
TOURNAMENT, CROSSOVER, MUTATE, SIGMA = 4, 0.7, 0.18, 0.12
TP, SL, MAX_HOLD = 0.03, 0.02, 2
...
IMMIGRANTS_PER_GEN = 6
TRANSACTION_COST = 0.0005  # 5 bps per round-trip
HOLDOUT_DD_PENALTY = 2000.0
HOLDOUT_RETURN_PENALTY = 60.0
...
def _init_worker(context: Tuple[Dict[str, pd.Series], Dict[str, Dict[str, pd.Series]]]):
    global price_data, feature_data
    price_ctx, feature_ctx = context
...
def simulate_equity(genome: np.ndarray, start: date, end: date) -> Tuple[np.ndarray, Dict[str, int]]:
    ...
    for date_val in dates:
        ...
        for pos in open_positions:
            idx = pos["i"]
            ticker = TICKERS[idx]
            if ticker not in price_data or ts not in price_data[ticker].index:
                still_open.append(pos)
                continue

            price_val = price_data[ticker].loc[ts]
            ...
            days_held = (ts - pos["date"]).days
            if pnl >= TP or pnl <= -SL or days_held >= MAX_HOLD:
                net_pnl = pnl - TRANSACTION_COST
                balance *= max(0.01, 1 + net_pnl)
                total_closed += 1
            else:
                still_open.append(pos)
        ...
        for i, ticker in enumerate(TICKERS):
            ...
            open_positions.append({"i": i, "entry": price, "date": ts, "short": signal < 0})
            total_opened += 1
            balance *= max(0.01, 1 - TRANSACTION_COST)
    ...
    return np.asarray(equity, dtype=float), {"closed": total_closed, "opened": total_opened}
...
def _compute_metrics(
    ...
    *,
    ...
    if worst_drawdown > MAX_DRAWDOWN_LIMIT:
        metrics["score"] = -200_000 - worst_drawdown * 100_000.0
        return metrics
...
def evaluate(genome: np.ndarray, eval_splits: Sequence[Tuple[date, date, date, date]] | None = None) -> float:
    attempts = 0
    ...
    while attempts < 3:
        ...
        metrics = _compute_metrics(genome, sampled, min_trades_override=required)
        score = metrics["score"]
...
def _evaluate_parallel(args):
    genome, eval_split_hint = args
    return genome, evaluate(genome, eval_split_hint)


def parallel_eval(genomes: Sequence[np.ndarray]):
    context = (price_data, feature_data)
    sampled_splits = _sample_splits(TRAIN_SPLITS, EVAL_SAMPLE_FRACTION)
    tasks = [(genome, sampled_splits) for genome in genomes]
...
def run(seed_best: np.ndarray | None = None):
    ...
    population = parallel_eval(population)
    ...
    for generation in range(1, GENS + 1):
        elites = [population[0][0], population[1][0]]
        offspring: List[np.ndarray] = elites.copy()

        while len(offspring) < POP_SIZE:
            parent1 = tournament(population)
            parent2 = tournament(population)
            child1, child2 = (
                crossover(parent1, parent2) if random.random() < CROSSOVER else (np.copy(parent1), np.copy(parent2))
            )
            mutate(child1)
            mutate(child2)
        ...
        if IMMIGRANTS_PER_GEN > 0:
            for i in range(IMMIGRANTS_PER_GEN):
                idx = len(offspring) - 1 - i
                if idx <= 1:  # preserve elites
                    break
                offspring[idx] = np.random.uniform(-1, 1, GENOME_LENGTH)

        population = parallel_eval(offspring)
        population.sort(key=lambda item: item[1], reverse=True)
