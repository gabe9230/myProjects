...
def _init_worker(context):
    global price_data, short_ma, long_ma, vol5, vol_spike, sentiment_data, geo_data
    (
        price_data,
        short_ma,
        long_ma,
...
def load_data():
    ...
    df["Combined"] = (
        df[[f"Top{i}" for i in range(1, 26)]].astype(str).agg(" ".join, axis=1)
    )
    df["Sentiment"] = df["Combined"].apply(lambda x: TextBlob(x).sentiment.polarity)
    df["Geo"] = df["Combined"].str.lower().apply(
        lambda x: sum(
            1
            for kw in [
                "war",
...
def simulate_equity(genome, start, end):
    ...
    for date_val in dates:
        ...
        for pos in open_pos:
            tkr, ts = TICKERS[pos["i"]], pd.Timestamp(date_val)
            # Skip if no price for this date/ticker (e.g., weekend or missing data)
            if tkr not in price_data or ts not in price_data[tkr].index:
            ...
            if pnl >= TP or pnl <= -SL or days >= MAX_HOLD:
                logging.info(
                    f"Closed {tkr} @ {ts.date()}: PnL={pnl:.4f}, held={days}d"
                )
                balance *= (1 + pnl)
                closed += 1
            else:
        ...
        for i, tkr in enumerate(TICKERS):
            ...
            # Skip tickers without loaded price data or missing date
            if tkr not in price_data or ts not in price_data[tkr].index:
                continue
            price = price_data[tkr].loc[ts]
            ...
            signal = float(
                ...
                + geo_w[i] * geo_data[tkr].get(ts, 0.0)
                + B
            )
            if any(p["i"] == i for p in open_pos):
                continue
            open_pos.append(
                {"i": i, "entry": price, "date": ts, "short": signal < 0}
            )
            opened += 1
            logging.info(
                f"Opened {tkr} @ {ts.date()} | Signal: {signal:.4f}"
            )
        eq.append(balance)

    if not eq or balance == 1000.0:
...
def mutate(g):
    for i in range(GENOME_LENGTH):
        if random.random() < MUTATE:
            g[i] = float(np.clip(g[i] + random.gauss(0, SIGMA), -1, 1))
...
def parallel_eval(genomes):
    ...
    with ProcessPoolExecutor(
        ...
        return [
            (g, f.result()) for g, f in zip(genomes, tqdm(futures, desc="Evaluating", ncols=80))
        ]


def run():
    pop = [np.random.uniform(-1, 1, GENOME_LENGTH) for _ in range(POP_SIZE)]
    pop = parallel_eval(pop)
    ...
    for gen in range(1, GENS + 1):
        ...
        while len(new) < POP_SIZE:
            ...
            c1, c2 = (
                crossover(p1, p2) if random.random() < CROSSOVER else (np.copy(p1), np.copy(p2))
            )
            mutate(c1)
            mutate(c2)
            new.append(c1)
...
if __name__ == "__main__":
    ...
    # Persist the genome for reuse by evaluation scripts
    np.save("best_genome.npy", best)
